...
def add_metronome_to_audio(osu_path, strong_beat_path, weak_beat_path, gain_db=0, progress_callback=None):
    ...
    metronome_audio = AudioSegment.silent(duration=len(audio))
    ...
    for i, (offset, bpm, time_signature) in enumerate(timing_points):
        interval = 60000 / bpm
        next_offset = timing_points[i + 1][0] if i + 1 < len(timing_points) else len(audio)
        current_time = offset

        while current_time < next_offset:
            for beat in range(time_signature):
                beat_time = current_time + (beat * interval)
                if beat_time < next_offset:
                    sound = primary_tick if beat == 0 else secondary_tick
                    metronome_audio = metronome_audio.overlay(sound, position=beat_time)
            current_time += time_signature * interval

        current_progress = offset / total_duration
